...
def polarity_plot(polarity_df):
    z = np.asarray(polarity_df)
    plt.figure(figsize=(15, 10))

    fig = make_subplots(
        rows=2, cols=1,
        subplot_titles=(
            "Heatmap of user sentiments(Polarity)",
            "Sentiment Distribution(Polarity)"))
    # Heatmap
    fig.add_trace(go.Heatmap(
        z=[z],
        type='heatmap', name='Polarity',
...
def subjectivity_plot(subjectivity_df):
    ...
    fig.add_trace(go.Heatmap(
        ...
        colorscale='Viridis', zmax=1, zmin=0,
...
def eda_on_tweet(user_name, tweet_count):
    if user_name != "" and tweet_count > 0:
        ...
        # polarity plot
        pol_img = polarity_plot(tweet_df['polarity'])
        st.subheader("Polarity of the tweets")
        st.plotly_chart(pol_img)

        #subjectivity plotly
        subj_img = subjectivity_plot(tweet_df['subjectivity'])
        st.subheader("Subjectivity of the tweets")
        st.plotly_chart(subj_img)
